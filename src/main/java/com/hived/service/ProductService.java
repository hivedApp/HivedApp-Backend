	...
	public String updateProductReview(ProductPojo productPojo) {
		...
		try {
			...
			ScanSpec scanSpec = new ScanSpec()
					.withProjectionExpression("#pk, userName,add_product_Photos,product_rating,product_title,product_description")
			...
			while (iter.hasNext()) {
				...
				String dateString = simpleDateFormat.format(new Date());
				UUID uuid = UUID.randomUUID();
				Item itm = new Item().withPrimaryKey("PK", uuid.toString())
						.withString("SK", productPojo.getUserId()).withString("entityType", "product_post")
						.withString("userName", item.getString("userName"))
						...
						.withString("title", item.getString("product_title"))
						.withString("creationDate", dateString);
				table.putItem(itm);
			}
...
//				.withProjectionExpression("#pk, userName,add_product_Photos,product_rating,product_title,product_description")
//				.withFilterExpression("#pk =:ema").withNameMap(new NameMap().with("#pk", "PK"))
//				.withValueMap(new ValueMap().withString(":ema", "6c2be91f-be33-406a-b585-91e51de47050"));
//
//		try {
//			ItemCollection<ScanOutcome> items = table.scan(scanSpec);
//
...
//				UUID uuid = UUID.randomUUID();
//				Item itm = new Item().withPrimaryKey("PK", uuid.toString())
//						.withString("SK", "12").withString("entityType", "product_post")
//						.withString("userName", item.getString("userName"))
//						.withList("images", item.getList("add_product_Photos"))
//						.withString("profile_url", "https://my-profile-image.s3.ap-south-1.amazonaws.com/facebook2.jpg")
//						.withString("rating", item.getString("product_rating"))
